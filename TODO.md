...
### EU-T040 页脚版本更新历史（v2.9.4）
**状态**：✅ 已完成并部署
**完成时间**：2026-08-07

- [x] 在全局页脚加入可展开的版本更新历史
- [x] 补充 2026-06-10 起的早期项目里程碑，避免误解为 7 月才开始建设
- [x] 完成中英德三语文案和移动端布局
- [x] 完成线上页面、静态资源和 API 健康检查验证

## 已完成 ✅

### EU-T039 问卷填写辨识与返回修改（v2.9.3）
**状态**：✅ 已完成并部署
**完成时间**：2026-08-04

- [x] 使用中性说明区和蓝色操作区区分说明内容与用户需要选择、填写或确认的内容
- [x] 产品/系列名称增加常驻示例“例如：F66 骑行头盔系列”
- [x] 已完成步骤整块可点击返回修改，不增加“重新编辑”按钮，并保留已填数据
- [x] 支持 Enter / Space 键重新展开已完成步骤，并同步用户引导位置
- [x] ESLint 0 errors（保留 26 条既有 warnings）、生产构建、国际化、安全测试 19 项、邮件测试 6 项与 diff 检查通过
- [x] 提交 `be08ddb`、标记 Git tag `v2.9.3`、推送并完成腾讯云部署验证
- [ ] 使用真实企业账号复核桌面端和移动端问卷视觉与返回修改流程

### EU-T038 ESLint 与用户引导体验收口（v2.9.2）
**状态**：✅ 已完成并部署
**完成时间**：2026-08-04

- [x] 新增 ESLint flat config，并使用兼容当前 Node.js 的 ESLint 9.39.5
- [x] 修复静态检查发现的未定义变量、旧企业页 Logo 上传处理和正则冗余转义
- [x] UG-01：稍后处理和成功删除后明确结束指引，保留页面结果提示
- [x] UG-03：不实现问卷断点恢复，入口统一显示“重新开始”
- [x] UG-04：只展开当前问卷步骤，固定弹窗标题，并在渲染后滚动到相邻步骤
- [x] UG-05：多公司账号上传前先提示确认资料归属公司
- [x] 批量导入按产品型号和文件名主体生成候选组，不再按资料类型拆分同一产品
- [x] 单份资料和拆分后的资料直接确认产品归属，多份候选组只判断一次是否属于同一产品
- [x] ESLint 0 errors（保留 26 条既有 warnings）、生产构建、国际化、安全测试和邮件测试通过
- [x] 提交 `43baa43`、Git tag `v2.9.2` 已推送，并完成腾讯云部署及线上自动化验证
- [ ] 使用真实企业账号回归单文件、多语言、多类型、不同型号和拆分路径

### EU-T037 安全依赖与发布版本收口（v2.9.1）
**状态**：✅ 已完成并部署
**完成时间**：2026-07-30

- [x] Multer 升级至 2.2.0，并更新 Body Parser 间接依赖
- [x] 同步升级服务器依赖锁文件与 Sharp 0.35.3，服务器生产依赖审计为 0 项漏洞
- [x] React Router 升级至当前最新版 7.18.2；RSC 审计项对当前纯客户端应用不适用，等待上游修复
- [x] `package.json`、`package-lock.json`、`.version` 和 Changelog 统一为 v2.9.1
- [x] 生产环境默认禁用流程清理 API、用户引导审阅页和 Stacklands 测试页
- [x] 新增中、英、德三语 404 页面
- [x] 公司详情页复用全局版权文案，不再硬编码 2025 年
- [x] 完成本地构建、国际化、安全测试和生产入口检查
- [x] 前端与版本收口提交 `b3ef5ff` 已推送至 GitHub `main`
- [x] 部署 v2.9.1；数据库备份、PM2、API、静态资源、404、测试入口和公司页版权验证通过
